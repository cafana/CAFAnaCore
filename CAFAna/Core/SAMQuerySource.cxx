...
namespace ana
{
  ...
  //----------------------------------------------------------------------
  std::string SAMQuerySource::SAMExperiment() const
  {
    char* strPtr = 0;
    strPtr = getenv("SAM_EXPERIMENT");
    if(strPtr) return strPtr;
    strPtr = getenv("EXPERIMENT");
    if(strPtr) return strPtr;
    return "cafana"; // unknown
  }
  ...
  std::string SAMQuerySource::EnsureDataset(const std::string& query) const
  {
    ...
    if(!SAMDefinitionExists(SAMExperiment(), dset.Data())){
      // I would be much much happier to do this in proper code, but I'm not
      // sure how, there's no samweb C++ API?
      system(TString::Format("samweb -e %s create-definition %s %s",
                             SAMExperiment().c_str(),
                             dset.Data(),
                             query.c_str()).Data());
    }
  ...
  {
    if(!SAMDefinitionExists(SAMExperiment(), def)){
      std::cout << "SAMQuerySource::EnsureSnapshot(): definition '"
                << def << "' not found." << std::endl;
      abort();
    }
    ...
    if(system(TString::Format("samweb -e %s create-definition %s file_name %s",
      ...
      double period = 1;
      while(!SAMDefinitionExists(SAMExperiment(), snap)){
        sleep(int(period));
        period *= 1.5;
        if(period > 60*30){
  ...
  LocationsForSAMQuery(const std::string& str, int stride, int offset, int limit)
  {
    TString query = str;
    ...
    if(limit > 0)
      query += TString::Format(" with limit %d", limit).Data();
