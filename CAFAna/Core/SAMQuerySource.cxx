...
namespace ana
{
  ...
  std::string SAMQuerySource::EnsureDataset(const std::string& query) const
  {
    ...
    if(!SAMDefinitionExists(dset.Data())){
      // I would be much much happier to do this in proper code, but I'm not
      // sure how, there's no samweb C++ API?
      system(TString::Format("samweb create-definition %s %s",
                             dset.Data(), query.c_str()).Data());
    }
  ...
  std::string SAMQuerySource::EnsureSnapshot(const std::string& def) const
  {
    ...
    if(system(TString::Format("samweb create-definition %s file_name %s",
      ...
      double period = 1;
      while(!SAMDefinitionExists(snap)){
        sleep(int(period));
        period *= 1.5;
        if(period > 60*30){
